...
import { clearDemoDatabase, deleteExample, getExamples, getPreferences, getSessions, replaceAll, saveExample, savePreferences, saveSession, type StorageNamespace } from './db';
...
function validId(id: string): boolean { return /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/.test(id); }

function routeInfo(url = new URL(location.href)): { route: Route; id?: string } {
  const parts = url.pathname.replace(/^\/+|\/+$/g, '').split('/').filter(Boolean);
  if (!parts.length) return url.searchParams.has('id') ? { route: 'not-found' } : { route: 'home' };
  const first = parts[0];
  if (parts.length !== 1 || !['demo', 'deck', 'edit', 'rehearse', 'sheet', 'settings', 'privacy', 'terms'].includes(first)) return { route: 'not-found' };
  const route = first as Route;
  const id = url.searchParams.get('id') ?? undefined;
  if (id && (route !== 'edit' && route !== 'rehearse' || !validId(id))) return { route: 'not-found' };
  if (!id && url.searchParams.has('id')) return { route: 'not-found' };
  return { route, id };
}

function routePath(route: Route, id?: string): string {
  if (route === 'home') return '/';
  return id && (route === 'edit' || route === 'rehearse') ? `/${route}?id=${encodeURIComponent(id)}` : `/${route}`;
}

function isDemoUrl(url = new URL(location.href)): boolean {
  return routeInfo(url).route === 'demo' || url.searchParams.get('demo') === '1';
}

function namespaceForCurrentUrl(): StorageNamespace { return isDemoUrl() ? 'demo' : 'real'; }
...
function demoView(): string {
  return `${pageHead('Sample deck', 'Try three interview examples', 'Choose an example and begin a 90-second rehearsal.', '<a class="button primary" href="/rehearse" data-route>Start sample rehearsal</a>')}
    <section class="card-grid" aria-label="Sample examples">${examples.map(example => `<article class="memory-card"><div class="card-top"><span class="evidence-dot"></span><span>${esc(example.role)}</span></div><h2>${esc(example.title)}</h2><p class="card-cue">“${esc(example.cue)}”</p><ul class="chips">${example.competencies.map(skill => `<li>${esc(skill)}</li>`).join('')}</ul><div class="card-actions"><a href="${routePath('edit', example.id)}" data-route>Edit sample<span class="sr-only"> ${esc(example.title)}</span></a><a href="${routePath('rehearse', example.id)}" data-route>Rehearse<span class="sr-only"> ${esc(example.title)}</span></a></div></article>`).join('')}</section>`;
}
...
function homeView(): string {
  ...
  return `<section class="hero">
    <div class="hero-copy"><p class="eyebrow"><span></span>Interview practice from your own work</p><h1 tabindex="-1">Recall your work examples in interviews</h1>
      ...
      <p class="action-note">See three sample cards and start a 90-second rehearsal.</p>
  ...
  <section class="snapshot" aria-labelledby="snapshot-title"><div><p class="kicker">Your deck now</p><h2 id="snapshot-title">Your deck at a glance</h2></div>
  ...
  <section class="boundary" aria-labelledby="privacy-title"><p class="kicker">Clear boundaries</p><h2 id="privacy-title">Your notes stay in this browser</h2><p>The app does not create interview answers, sync a cloud copy, or track how you use it. Your browser’s speech provider may receive the audio you dictate.</p><a href="/privacy" data-route>Read the privacy details</a></section>
  <section class="pricing" aria-labelledby="pricing-title"><p class="kicker">Optional one-time license</p><h2 id="pricing-title">Keep six examples for free</h2><p>Six examples, rehearsal, the recall sheet, accessibility controls, and exports stay free.</p><p>New $9 licenses are currently unavailable. Existing license holders can restore access in Settings.</p><a class="button quiet" href="/settings" data-route>Open Settings</a></section>`;
}
...
function deckView(): string {
  ...
  return `${pageHead('Your evidence', 'Your recall deck', `${examples.length} truthful ${examples.length === 1 ? 'example' : 'examples'}, ready to rehearse.`, action)}
    ${!canAdd ? '<div class="inline-note"><strong>Free deck complete.</strong> Keep rehearsing these six. New $9 licenses are currently unavailable.</div>' : ''}
    <section class="card-grid" aria-label="Saved examples">${examples.map(example => `<article class="memory-card">
      ...
      <div class="card-actions"><a href="${routePath('edit', example.id)}" data-route>Edit<span class="sr-only"> ${esc(example.title)}</span></a><a href="${routePath('rehearse', example.id)}" data-route>Rehearse<span class="sr-only"> ${esc(example.title)}</span></a></div>
    </article>`).join('')}</section>`;
}

function editorView(id?: string): string {
  const example = examples.find(item => item.id === id);
  if (!example && !unlocked && examples.length >= FREE_EXAMPLE_LIMIT) return `${pageHead('Deck limit', 'Your six examples are ready', 'The free deck remains fully usable. New $9 licenses are currently unavailable.')}<div class="glass-callout"><a class="button quiet" href="/deck" data-route>Back to deck</a></div>`;
...
function privacyView(): string {
  return `${pageHead('Effective 28 August 2026', 'Privacy, in plain words', 'Your interview examples stay in this browser. We do not receive, read, sell, or analyse them.')}
    <article class="legal-content"><h2>What this browser stores</h2><p>The app stores examples, preferences, and rehearsal history in IndexedDB. Sample data uses a separate database. Resetting the sample or starting for real deletes it.</p><h2>Exports</h2><p>Your browser encrypts each backup before download using AES-256-GCM. The passphrase is not stored. CSV files are readable, so keep them somewhere you trust.</p><h2>Network requests</h2><p>The app has no analytics, advertising, tracking pixels, third-party fonts, or remote scripts. Existing license checks contact only the Sociobot API. They never include deck content.</p><h2>Voice features</h2><p>Read-aloud uses device speech synthesis. Optional dictation uses browser speech recognition. Your browser’s speech provider may receive the audio you dictate.</p><h2>Questions</h2><p>Email <a href="mailto:privacy@sociobot.in">privacy@sociobot.in</a>. Do not include private interview notes.</p></article>`;
}
...
function bindEvents(): void {
  document.querySelectorAll<HTMLAnchorElement>('a[data-route]').forEach(anchor => anchor.addEventListener('click', event => {
    ...
    if (demoMode) target.searchParams.set('demo', '1');
...
function navigate(path: string, replace = false): void {
  ...
  if (demoMode) target.searchParams.set('demo', '1');
  if (!target.pathname.startsWith('/rehearse')) rehearsal = null;
  history[replace ? 'replaceState' : 'pushState']({}, '', `${target.pathname}${target.search}`);
  void synchronizeRoute(true);
}

async function loadNamespace(namespace: StorageNamespace): Promise<{ examples: Example[]; sessions: Session[]; preferences: Preferences }> {
  let [loadedExamples, loadedSessions, loadedPreferences] = await Promise.all([getExamples(namespace), getSessions(namespace), getPreferences(namespace)]);
  if (namespace === 'demo' && loadedExamples.length === 0) {
    await replaceAll(namespace, sampleExamples, []);
    loadedExamples = await getExamples(namespace);
    loadedSessions = [];
    loadedPreferences = await getPreferences(namespace);
  }
  return { examples: loadedExamples, sessions: loadedSessions, preferences: loadedPreferences };
}

async function synchronizeRoute(focusMain = false): Promise<void> {
  const transition = ++routeTransition;
  const namespace = namespaceForCurrentUrl();
  demoMode = namespace === 'demo';
  if (storageNamespace !== namespace) {
    app.innerHTML = `<main id="main" tabindex="-1"><h1 tabindex="-1">Opening ${namespace === 'demo' ? 'the sample deck' : 'your deck'}…</h1></main>`;
    try {
      const loaded = await loadNamespace(namespace);
      ...
      if (transition !== routeTransition) return;
      shell(`${pageHead('Storage issue', 'Your deck could not open', 'Private browsing or browser storage settings may be blocking this app. Allow site storage, then reload.')}<button class="button primary" onclick="location.reload()">Try again</button>`);
      return;
    }
  }
  if (transition !== routeTransition) return;
...
async function handleExampleSubmit(event: Event): Promise<void> {
  ...
  try { await saveExample(storageNamespace!, example); examples = await getExamples(storageNamespace!); showNotice(`${example.title} saved.`); navigate('/deck'); }
...
async function rateCurrent(rating: RecallRating): Promise<void> {
  ...
  if (rehearsal.index >= rehearsal.cards.length) {
    ...
    await saveSession(storageNamespace!, session); sessions = await getSessions(storageNamespace!); showNotice('Rehearsal complete and saved on this device.');
  }
  render();
}
...
async function handleAction(event: Event): Promise<void> {
  ...
  if (action === 'delete') {
    ...
    if (example && confirm(`Delete “${example.title}”? This cannot be undone.`)) { await deleteExample(storageNamespace!, example.id); examples = await getExamples(storageNamespace!); showNotice(`${example.title} deleted.`); navigate('/deck'); }
  }
  ...
  if (action === 'reset-demo' && storageNamespace === 'demo') { await replaceAll('demo', sampleExamples, []); preferences = { duration: 90, reduceMotion: false, ttsRate: 1 }; await savePreferences('demo', preferences); examples = await getExamples('demo'); sessions = []; rehearsal = null; navigate('/demo', true); showNotice('Demo reset to three sample examples.'); }
...
async function handlePreferences(event: Event): Promise<void> {
  ...
  await savePreferences(storageNamespace!, preferences); render(); showNotice('Comfort settings saved.');
}
...
async function handleImport(event: Event): Promise<void> {
  ...
  try {
    ...
    await replaceAll(storageNamespace!, imported.examples, imported.sessions); await savePreferences(storageNamespace!, imported.preferences); examples = await getExamples(storageNamespace!); sessions = await getSessions(storageNamespace!); preferences = await getPreferences(storageNamespace!); showNotice('Backup imported.'); navigate('/deck');
...
window.addEventListener('popstate', () => { speechSynthesis?.cancel(); if (routeInfo().route !== 'rehearse') rehearsal = null; void synchronizeRoute(true); });
